...
interface DmConv {
  ...
  members?: DmMember[]; muted?: boolean;
}
  ...
  return (
    <div className="flex justify-center bg-[var(--color-bg)] min-h-screen"
      ...
      <div className="w-full max-w-[430px] pb-20">
              ...
              <div>
                ...
                <div className="sticky top-[57px] z-30 bg-[var(--color-bg)]/98 backdrop-blur-md border-b border-[var(--color-line)]">
                  <div className="flex items-center justify-between px-4 py-3">
                    <button onClick={() => setShowDm(false)} className="font-display text-sm text-[var(--color-mute)] hover:text-[var(--color-ink)] transition">← 戻る</button>
                    <span className="font-display text-sm font-semibold">ダイレクトメッセージ</span>
                    <span className="w-12" />
                  </div>
                </div>
                {/* Search + Group create */}
                ...
                {dms.filter(d => !d.muted && (!dmSearch || d.name.includes(dmSearch))).map(d => (
                  <div key={d.id} className="flex items-center border-b border-[var(--color-line)] hover:bg-[var(--color-bg-soft)] transition">
                    <button
                    ...
                    >
                      <div className="relative flex-none">
                        ...
                        {d.muted && (
                          <span className="absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full bg-[var(--color-bg-soft)] border border-[var(--color-line)] flex items-center justify-center">
                            <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="var(--color-mute)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-baseline justify-between mb-0.5">
                          <span className={`font-display text-sm ${d.muted ? "text-[var(--color-mute)]" : d.unread > 0 ? "text-[var(--color-ink)]" : "text-[var(--color-ink-soft)]"}`}>{d.name}{d.muted && <span className="ml-1.5 font-display text-[9px] text-[var(--color-mute)] border border-[var(--color-line)] px-1 py-0.5 rounded">ミュート中</span>}</span>
                          <span className="font-display text-[10px] text-[var(--color-mute)] flex-none ml-2">{d.time}</span>
                        </div>
                        <p className={`text-xs truncate ${d.muted ? "text-[var(--color-mute)] opacity-50" : "text-[var(--color-mute)]"}`}>{d.lastMsg}</p>
                      </div>
                    </button>
                    <button
                      onClick={e => { e.stopPropagation(); setDmMenuTarget(d); }}
                      className="flex-none px-3 py-4 text-[var(--color-mute)] hover:text-[var(--color-ink)] transition"
                    >
                  ...
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 mt-3">
                        ...
                        {p.isOwn ? (
                          <button
                            onClick={() => p.likes > 0 && setLikersPost(p)}
                            className="flex items-center gap-1 text-xs transition"
                            style={{ color: p.likes > 0 ? "#f87171" : "var(--color-mute)", cursor: p.likes > 0 ? "pointer" : "default" }}>
                            <svg width="15" height="15" viewBox="0 0 24 24" fill={p.likes > 0 ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                          ...
                          <button
                            ...
                            {p.liked && <span className="num">{p.likes}</span>}
                          </button>
                        )}
        ...
        {/* ===== いいね一覧モーダル ===== */}
        {likersPost && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60" onClick={() => setLikersPost(null)}>
            <div className="w-full max-w-[430px] bg-[var(--color-bg-soft)] rounded-t-3xl pt-4 pb-24 max-h-[70vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <div className="w-10 h-1 bg-[var(--color-line)] rounded-full mx-auto mb-4" />
              <div className="px-5 mb-4 flex items-center gap-2">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="#f87171" stroke="#f87171" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
                </svg>
                <span className="font-display text-base font-semibold">いいねした人 {likersPost.likes}人</span>
              </div>
              <div className="px-5 space-y-1">
                {[
                  { name:"田中 康介", handle:"@tanaka_k", avatar:"/images/tanaka.png", rank:"GOLD" },
                  { name:"山本 彩花", handle:"@yamamoto_a", avatar:"/images/yamamoto.png", rank:"PLATINUM" },
                  { name:"伊藤 健", handle:"@ito_k", avatar:"/images/ito.png", rank:"SILVER" },
                  { name:"森田 桂", handle:"@morita_k", avatar:"森", rank:"GOLD" },
                  { name:"鈴木 花", handle:"@suzuki_h", avatar:"鈴", rank:"SILVER" },
                ].slice(0, likersPost.likes).map((u, i) => (
                  <div key={i} className="flex items-center gap-3 py-2.5 border-b border-[var(--color-line)] last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="font-display text-[10px] text-[var(--color-mute)]">{u.handle}</div>
                    </div>
                    <span className="tag text-[9px] px-2 py-0.5">{u.rank}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* ===== DM Menu Bottom Sheet ===== */}
        {dmMenuTarget && (
          <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60" onClick={() => setDmMenuTarget(null)}>
            <div className="w-full max-w-[430px] bg-[var(--color-bg-soft)] rounded-t-3xl pt-4 pb-24" onClick={e => e.stopPropagation()}>
              <div className="w-10 h-1 bg-[var(--color-line)] rounded-full mx-auto mb-5" />
              <div className="px-5 mb-4">
                <div className="flex items-center gap-3">
                  <Avatar src={dmMenuTarget.avatar} name={dmMenuTarget.name} size={10} />
                  <div>
                    <div className="font-display text-sm text-[var(--color-ink)]">{dmMenuTarget.name}</div>
                    {!dmMenuTarget.isGroup && <div className="font-display text-[10px] text-[var(--color-mute)]">@{dmMenuTarget.handle}</div>}
                  </div>
                </div>
              </div>
              <div className="px-4 space-y-2">
                ...
                <button
                  onClick={() => {
                    setDms(prev => prev.map(d => d.id === dmMenuTarget.id ? { ...d, muted: !d.muted } : d));
                    setDmMenuTarget(null);
                  }}
                  className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl border border-[var(--color-line)] bg-[var(--color-bg)] hover:bg-[var(--color-bg-soft)] transition text-left"
                >
                  <div className="w-9 h-9 rounded-full flex items-center justify-center flex-none" style={{ background: "rgba(142,138,128,0.12)" }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-mute)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      {dmMenuTarget.muted
                        ? <><path d="M18 8a6 6 0 0 1 .2 1.6V12a6 6 0 0 1-6 6 6 6 0 0 1-5.68-4"/><path d="M9 9v3a3 3 0 0 0 4.5 2.6"/><path d="M17 17H3v-2a6 6 0 0 1 .34-2"/><path d="M15 9V4a3 3 0 0 0-5.94-.6"/><path d="M12 19v3M8 19h8"/><line x1="1" y1="1" x2="23" y2="23"/></>
                        : <><path d="M18 8a6 6 0 0 0-9.33-5"/><path d="M9.17 9.17A6 6 0 0 0 6 12v2"/><path d="M17 17H3v-2a6 6 0 0 1 9.33-5"/><path d="M12 19v3M8 19h8"/></>
                      }
                    </svg>
                  </div>
                  <div>
                    <div className="font-display text-sm text-[var(--color-ink)]">
                      {dmMenuTarget.muted ? "ミュートを解除" : "ミュートする"}
                    </div>
                    <div className="font-display text-[10px] text-[var(--color-mute)] mt-0.5">
                      {dmMenuTarget.muted ? "通知を再開します" : "通知をオフにします"}
                    </div>
                  </div>
                </button>
                {/* 削除する */}
                <button
                  onClick={() => {
                    setDms(prev => prev.filter(d => d.id !== dmMenuTarget.id));
                    setDmMenuTarget(null);
                  }}
                  className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl border border-[var(--color-line)] bg-[var(--color-bg)] hover:bg-[var(--color-bg-soft)] transition text-left"
                >
                  ...
                  <div>
                    <div className="font-display text-sm" style={{ color: "#ef4444" }}>削除する</div>
                    <div className="font-display text-[10px] text-[var(--color-mute)] mt-0.5">トークを削除します</div>
                  </div>
                </button>
                {/* 通報（グループ以外） */}
                {!dmMenuTarget.isGroup && (
                  <button
                    onClick={() => { setReportSheet(dmMenuTarget); setDmMenuTarget(null); }}
                    className="w-full flex items-center gap-4 px-4 py-4 rounded-2xl border border-[var(--color-line)] bg-[var(--color-bg)] hover:bg-[var(--color-bg-soft)] transition text-left"
                  >
                    <div className="w-9 h-9 rounded-full flex items-center justify-center flex-none" style={{ background: "rgba(239,68,68,0.1)" }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#ef4444" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <div>
                      <div className="font-display text-sm" style={{ color: "#ef4444" }}>通報する</div>
                      <div className="font-display text-[10px] text-[var(--color-mute)] mt-0.5">運営に報告します</div>
                    </div>
                  </button>
                )}
                <button
                  onClick={() => setDmMenuTarget(null)}
                  className="w-full py-3.5 rounded-2xl font-display text-sm text-[var(--color-mute)] hover:text-[var(--color-ink)] border border-[var(--color-line)] transition mt-2"
                >
                  キャンセル
                </button>
              </div>
            ...
            <div className="w-full max-w-[430px] bg-[var(--color-bg-soft)] rounded-t-3xl pt-4 pb-24" onClick={e => e.stopPropagation()}>
              ...
              {reportedDms.has(reportSheet.id) ? (
                <div className="px-5 py-6 text-center space-y-3">
                  ...
                  <button onClick={() => setReportSheet(null)} className="mt-2 w-full py-3.5 rounded-full font-display text-sm border border-[var(--color-line)] text-[var(--color-mute)] transition hover:text-[var(--color-ink)]">閉じる</button>
                </div>
              ) : (
                <div className="px-4 space-y-2">
                  {reportReason ? (
